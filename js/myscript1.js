              ...
              this.container.y=this.body.GetWorldCenter().y*SCALE;
                ...
                if(theoldContainer!=null){
                        Tween.get(theoldContainer.getChildAt(1)).to({scaleX:1,scaleY:1},1000);
                         if(text!=null){
                    console.log(text);
                    Tween.get(text).to({lineHeight:0},5000);
                    stage.removeChild(text);
                   }
                        Tween.get(theoldContainer.getChildAt(0)).to({alpha:1});
                        Tween.get(theoldContainer).to({x:415,y:510},3000).wait(1000).to({x:870,y:510,rotation:360},3000).to({alpha:0});
                    }
                Tween.get(this.container).to({x:270,y:100,rotation:160},2000).to({x:415,y:100,rotation:0}, 3000,Ease.bounceOut).call(function(){
                    
                   Tween.get(thegreatcontainer.getChildAt(0)).to({alpha:0},2000);
                  
                    text = new Text("Android Development", "24px Arial", "#3296BB");
	        text.x = 600;
	        text.y = 100;
