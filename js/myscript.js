var vivek={}
vivek.irs=(function(){
    var canvas,context,themainshape,theoldContainer,text;
    var bodies=[];
    ...
    var stagesetup=(function(){
        ...
        var topBorderLine=new Graphics();
        topBorderLine.setStrokeStyle(1);
        topBorderLine.beginStroke(Graphics.getRGB(51,181,229));
        topBorderLine.beginFill(Graphics.getRGB(255,0,0));
        topBorderLine.moveTo(0,0).lineTo(870, 0);
        topBorderLine.moveTo(0,550).lineTo(870,550);
        topBorderLine.moveTo(15,250).lineTo(15,550);
        topBorderLine.moveTo(315,550).lineTo(315,250);
        var s = new Shape(topBorderLine);
    ...
    var theGreatCircles=(function(){
        ...
        var spawn=function(){
            ...
            cirBmp.regX=40;
            cirBmp.regY=40;
            var andBmp=new Bitmap("img/android.png");
            andBmp.scaleX=0.372;
            andBmp.scaleY=0.372;
            andBmp.regX=58;
            andBmp.regY=72;
            ...
            container.x=Math.round(30+Math.random()*180);
            //cirBmp.y=550;
            container.y=-3;
    ...
    var myphysics=(function(){
        ...
        var setup=function(){
            ...
            //add floor
                ...
                floorfixture.shape.SetAsBox(435/SCALE,1/SCALE);
                //->then define body
                var floorbodydef=new thebodydef;
                floorbodydef.type=thebody.b2_staticBody;
                floorbodydef.position.x=435/SCALE;
                floorbodydef.position.y=549/SCALE;
            ...
            this.update=function(i){
              ...
              this.container.y=this.body.GetWorldCenter().y*SCALE;
                ...
                if(theoldContainer!=null){
                    ...
                    $("#thedesctext").empty();
                        Tween.get(theoldContainer.getChildAt(1)).to({scaleX:0.372,scaleY:0.372},1000);
                    
                        Tween.get(theoldContainer.getChildAt(0)).to({alpha:1});
                        Tween.get(theoldContainer).to({x:415,y:510},1000).to({x:870,y:510,rotation:360},3000).to({alpha:0});
                    }
                Tween.get(this.container).to({x:165,y:250,rotation:0},2000,Ease.none).to({x:415,y:200},3000,Ease.none).call(function(){
                    
                   Tween.get(thegreatcontainer.getChildAt(0)).to({alpha:0},2000);
                   Tween.get(thegreatcontainer.getChildAt(1)).to({scaleY:1,scaleX:1},2000);
                  $("#theheading").text("Android Development");
                 
                  $("#thedesctext").text("IRS Soft caters to every needof its clients globally in application development spanning across various technologies such as Java,C#,VB.Net,C++ and MS technologies.While Developing enterprise solutions using Java,C#,C++ and VB.Net , we also undertake projects in Data Integration, Data Warehousing using Tibco,Cognos and SAS etc.Besides,Security and Network Management applications are developed at IRS Soft with aplomb using TCP/IP and other protocols.At IRS Soft,clients are assured of the best quality benchmarks and strictly adhere to the promised time schedules");
                    
                });
                theoldContainer=this.container;
                //Tween.get(thegreatlogo).wait(3000).to({scaleX:1.5,scaleY:1.5},3000); 
       ...
       bodyfixturedef.shape=new theCircleShape(40/SCALE);
